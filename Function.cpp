...
void create_knf(int **Arr, int **knf) //Вывод таблицы истинности в файл
{
	//10 на 512
	int num = 0, dva = 256;
	for (int i = 0; i < 512; i++) //заполнение под значения переменных
	{
		for (int j = 0; j < 9; j++)
		{
			if ((num & dva) != 0)
	...
	for (int k = 0; k < 512; k++) //получение значения уравнения 10 заменить на 256
	{
		...
		if (result!= 1)
		{
			for (int j = 56; j < 84; j++)
			{
				...
				if (knf[k][4] == 0)
					e = 0;
				//_____________________________________________________________________________
				if ((Arr[j][4] == 1) && (knf[k][5] == 1))
					f = 0;
				if ((Arr[j][4] == 1) && (knf[k][5] == 0))
					f = 1;
				if ((Arr[j][4] == 0) && (knf[k][5] == 1))
					f = 1;
				if ((Arr[j][4] == 0) && (knf[k][5] == 0))
					f = 0;
				//_____________________________________________________________________________
				if ((Arr[j][5] == 1) && (knf[k][6] == 1))
					g = 0;
				if ((Arr[j][5] == 1) && (knf[k][6] == 0))
					g = 1;
				if ((Arr[j][5] == 0) && (knf[k][6] == 1))
					g = 1;
				if ((Arr[j][5] == 0) && (knf[k][6] == 0))
					g = 0;
				//_____________________________________________________________________________
				if ((Arr[j][6] == 1) && (knf[k][7] == 1))
					h = 0;
				if ((Arr[j][6] == 1) && (knf[k][7] == 0))
					h = 1;
				if ((Arr[j][6] == 0) && (knf[k][7] == 1))
					h = 1;
				if ((Arr[j][6] == 0) && (knf[k][7] == 0))
					h = 0;
				//_____________________________________________________________________________
				if ((Arr[j][7] == 1) && (knf[k][8] == 1))
					i = 0;
				if ((Arr[j][7] == 1) && (knf[k][8] == 0))
					i = 1;
				if ((Arr[j][7] == 0) && (knf[k][8] == 1))
					i = 1;
				if ((Arr[j][7] == 0) && (knf[k][8] == 0))
					i = 0;
